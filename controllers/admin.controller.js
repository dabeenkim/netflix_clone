...
class AdminController {
    ...
    postMovie = async(req,res) => {
        try {
            const {title, category, desc, playtime, actor, genre, thumbUrl, movieUrl} = req.body;
            ...
            await this.adminService.postMovie({title, category, desc, playtime, actor, genre, thumbUrl, movieUrl});
    ...
    updateMovie = async(req, res) => {
        try {
            ...
            if(movieId) { updatedElement.movieId = movieId}
            if(title) { updatedElement.title = title}
            if(category) { updatedElement.category = category}
            if(desc) { updatedElement.desc = desc}
            if(playtime) { updatedElement.playtime = playtime}
            if(actor) { updatedElement.actor = actor}
            if(genre) { updatedElement.genre = genre}
            if(thumbUrl) { updatedElement.category = thumbUrl}
            if(movieUrl) { updatedElement.category = movieUrl}

            await this.adminService.updateMovie(updatedElement);
            return res.status(201).json({updatedMovie, message: '정보 수정을 완료했습니다.'});
        } catch (error) {
            if (Boom.isBoom(error)) {
                res.status(error.statusCode).json({ errorMessage: error.message }); // 에러 메시지를 설정하면 이쪽으로 빠집니다.
              } else {
